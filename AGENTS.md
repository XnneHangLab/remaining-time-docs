...
| 主要系统 | 介绍货币系统、时间流逝系统、收藏系统、任务系统的设计目的、玩家可见规则与体验 |

- 当前不发布游戏架构、JSON 的具体字段说明或详细源码设计。不要把源文件字段表、接口清单、实现步骤或原始技术合同复制为网站文章。
- 编剧 Skill 按面向编剧的说明组织；涉及内部路径、JSON 字段或源码实现的部分不直接搬运到网站。未实现的能力不能写成已支持。
- 项目处于高速迭代期，不为每次代码改动自动新增实现文档，不创建需要不断同步的源码说明副本；只在上述内容发生实质变化时更新相应页面。
- 游戏仓库顶层 `docs/` 暂停新增与扩写，已有文件保留供查阅；本仓库 `docs/` 是 website 的页面目录，按上述范围编写。
